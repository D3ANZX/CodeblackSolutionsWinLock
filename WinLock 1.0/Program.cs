    ...
    internal static class Program
    {
        ...
        public static void Main()
        {
            ...
            using (LoginForm newSession = new LoginForm())
            {
                ...
                {
                    newSession.ApplyFullLockdown();
                }
                if (newSession.ShowDialog() == DialogResult.OK)
                {
                    
                    Application.Run(new WinLock(newSession.username, newSession.role, newSession.authFilePath, newSession.hashedInputPassword, newSession.CurrentUserExpiryString));
